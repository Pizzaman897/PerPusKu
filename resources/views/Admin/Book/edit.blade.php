        ...
        <div class="flex-1">
            ...
            <main class="p-8">
                <div class="mx-auto max-w-2xl rounded-2xl bg-white p-8 shadow-sm">
                    ...
                    <h1 class="mb-6 text-2xl font-bold">Edit Buku</h1>
                    ...
                    <form method="POST" action="{{ route('admin.buku.update', $id) }}" class="space-y-5">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <select name="category" id="category"
                                ...
                                <option value="" disabled @selected(old('category', $buku->category) === null)>Jenis buku...</option>
                                <option value="fiksi" @selected(old('category', $buku->category) === 'fiksi')>Fiksi</option>
                                <option value="non-fiksi" @selected(old('category', $buku->category) === 'non-fiksi')>Non-fiksi</option>
                                <option value="pelajaran" @selected(old('category', $buku->category) === 'pelajaran')>Pelajaran</option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="4"
                                      ...
                                      class="w-full rounded-lg border border-slate-300 bg-indigo-50/60 px-4 py-2.5 text-sm placeholder-slate-500 focus:border-indigo-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-200">{{ old('description', $buku->description) }}</textarea>
                        </div>

                        <div>
                            <label for="image_url" class="mb-1.5 block text-sm font-semibold">URL</label>
                            <input type="text" name="image_url" id="image_url"
                                   value="{{ old('image_url', $buku->image_url) }}"
                                   placeholder="url..."
                                   class="w-full rounded-lg border border-slate-300 bg-indigo-50/60 px-4 py-2.5 text-sm placeholder-slate-500 focus:border-indigo-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-200">
                        </div>
                        ...
                        <div>
                            ...
                            <select name="status" id="status"
                                    class="w-full rounded-lg border border-slate-300 bg-indigo-50/60 px-4 py-2.5 text-sm focus:border-indigo-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-200">
                                <option value="" disabled @selected(old('status', $buku->status) === null)>Status buku...</option>
                                <option value="tersedia" @selected(old('status', $buku->status) === 'tersedia')>Tersedia</option>
                                <option value="dipinjam" @selected(old('status', $buku->status) === 'dipinjam')>Dipinjam</option>
                            </select>
                        </div>
