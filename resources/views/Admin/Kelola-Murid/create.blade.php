...
<head>
    ...
    <title>Tambah Murid - PerPusKu</title>
    ...
    <div class="flex min-h-screen">
        ...
        <main class="flex-1 p-8">
            <div class="w-full rounded-2xl bg-white p-10 shadow-sm">
                ...
                <h1 class="mb-8 text-3xl font-extrabold text-[#051541]">Tambahkan Murid</h1>
                ...
                <form method="POST" action="{{ route('admin.kelola-murid.store') }}" class="max-w-xl space-y-6">
                    @csrf

                    <div>
                        <label for="name" class="mb-2 block text-base font-bold text-[#051541]">Murid</label>
                    ...
                    <div>
                        <label for="status_pinjam" class="mb-2 block text-base font-bold text-[#051541]">Status pinjam</label>
                        <select name="status_pinjam" id="status_pinjam"
                                class="w-full rounded-lg border-2 border-[#051541] bg-[#EDF1FD] px-4 py-3 text-base text-[#051541] focus:border-[#4748D9] focus:outline-none focus:ring-2 focus:ring-[#4748D9]/30">
                            <option value="" {{ old('status_pinjam') ? '' : 'selected' }} disabled>Status...</option>
                            <option value="dipinjam" {{ old('status_pinjam') === 'dipinjam' ? 'selected' : '' }}>Sedang Meminjam</option>
                            <option value="tidak_dipinjam" {{ old('status_pinjam') === 'tidak_dipinjam' ? 'selected' : '' }}>Tidak Meminjam</option>
                        </select>
                    </div>
