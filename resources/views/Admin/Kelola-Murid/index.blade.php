...
<head>
    ...
    <title>Kelola Murid - PerPusKu</title>
    ...
    <div class="flex min-h-screen">
        ...
        <main class="flex-1 p-8">
            ...
            <div class="mb-6 flex items-center justify-between">
                <h1 class="text-3xl font-extrabold text-[#051541]">Kelola Murid</h1>
                <a href="{{ route('admin.kelola-murid.create') }}"
                   class="flex items-center gap-2 rounded-xl bg-[#4748D9] px-5 py-3 font-semibold text-white shadow hover:bg-[#151B72] transition">
                    <span class="text-lg leading-none">+</span> Tambahkan Murid
                </a>
            </div>
            ...
            <div class="overflow-hidden rounded-2xl bg-white shadow-sm">
                <table class="w-full text-left">
                    <thead>
                        <tr class="bg-[#EDF1FD] text-[#051541]">
                            <th class="px-6 py-4 font-bold">No</th>
                            <th class="px-6 py-4 font-bold">Murid</th>
                    ...
                    <tbody>
                        @forelse ($students as $student)
                            <tr class="{{ $loop->iteration % 2 === 0 ? 'bg-[#EDF1FD]/60' : 'bg-white' }}">
                                <td class="px-6 py-4 font-semibold text-[#051541]">
                                    {{ $loop->iteration }}.
                                </td>
                                <td class="px-6 py-4 text-gray-800">
                                    <a href="{{ route('admin.kelola-murid.show', $student['id']) }}" class="hover:text-[#4748D9] hover:underline">
                                        {{ $student['name'] }}
                                    </a>
                                </td>
                                <td class="px-6 py-4 text-gray-800">{{ $student['email'] }}</td>
                                <td class="px-6 py-4 text-gray-800">{{ $student['nis'] }}</td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-end gap-3">
                                        {{-- Tombol Edit --}}
                                        <a href="{{ route('admin.kelola-murid.edit', $student['id']) }}"
                                           class="text-[#051541] hover:text-[#4748D9]" title="Edit">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
